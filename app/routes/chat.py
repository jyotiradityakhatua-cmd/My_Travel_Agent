...
async def chat(
    ...
    async def stream():
        ...
        save_message(db=db, chat_id=chat_id, role="assistant", message=full)
        # Embed chat_id so the frontend can track the session
        yield f"\n\n<!--CHAT_ID:{chat_id}-->"
...
async def search_places(
    q: str = Query(..., description="Search term, e.g. 'Casa Frei Goa'"),
    lat: Optional[float] = Query(None),
    lng: Optional[float] = Query(None),
    zoom: int = Query(13),
):
    ...
    params: dict = {"engine": "google_maps", "q": q, "api_key": SEARCHAPI_KEY}
    if lat is not None and lng is not None:
        params["ll"] = f"@{lat},{lng},{zoom}z"
...
async def search_places_batch(
    ...
    for result in results:
        ...
        if result.get("places"):
            all_places.append(result["places"][0])
    return {"places": all_places}